...
# -------------------------------------------------------------------
# TinyViT + TransformerBlock (same as before)
# -------------------------------------------------------------------
...
class TinyBrainTumorClassifier(pl.LightningModule):
    def __init__(self, mae_ckpt_path=None, num_classes=4, lr=1e-3):
        ...
        # Load MAE encoder weights if provided
        if mae_ckpt_path:
            ...
            # Extract encoder part (keys start with 'encoder.')
...
if __name__ == '__main__':
    # Find the multi‑modal MAE checkpoint automatically
    ...
    trainer = pl.Trainer(
        max_epochs=10,          # a few more epochs with good initialisation
        accelerator='cpu',
        devices=1,
